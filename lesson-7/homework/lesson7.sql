...
--Task 15:
select DepartmentName, avg(Salary) as avg_salary, count(*) from Employees
group by DepartmentName
...
--Task 24:
select month(SaleDate), sum(SaleAmount), count(distinct ProductID) from Sales
group by month(SaleDate)
having COUNT(DISTINCT ProductID) > 8;

select * from Orders
--Task 25:
select year(OrderDate), min(Quantity) as min_Quantity, max(Quantity) as max_Quantity from Orders
