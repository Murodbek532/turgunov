...
--Task 7:
SELECT c.FirstName, o.MostRecentOrderDate, o.OrderID
FROM Customers c
LEFT JOIN (
...
WHERE (d.DepartmentName IN ('HR', 'Finance'))
   OR (LENGTH(REPLACE(LOWER(e.Name), 'a', '')) - LENGTH(e.Name) +
        LENGTH(REPLACE(LOWER(e.Name), 'e', '')) - LENGTH(e.Name) +
        LENGTH(REPLACE(LOWER(e.Name), 'i', '')) - LENGTH(e.Name) +
        LENGTH(REPLACE(LOWER(e.Name), 'o', '')) - LENGTH(e.Name) +
        LENGTH(REPLACE(LOWER(e.Name), 'u', '')) - LENGTH(e.Name) >= 4);
