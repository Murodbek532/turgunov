...
--Task 8:
select * from person
where id != null or ssn != null or passportid != null or itin != null 
...
--Task 10:
select EMPLOYEE_ID, FIRST_NAME, LAST_NAME, DEPARTMENT_ID from Employees as e
...
Chars AS (
    SELECT SUBSTRING(@input, n, 1) AS ch
    FROM Numbers
),
Classified AS (
    SELECT 
        ch,
        CASE 
            WHEN ch COLLATE Latin1_General_CS_AS LIKE '[A-Z]' THEN 'Upper'
            WHEN ch COLLATE Latin1_General_CS_AS LIKE '[a-z]' THEN 'Lower'
...
SELECT
    STRING_AGG(CASE WHEN Category = 'Upper' THEN ch END, '') AS UppercaseLetters,
    STRING_AGG(CASE WHEN Category = 'Lower' THEN ch END, '') AS LowercaseLetters,
    STRING_AGG(CASE WHEN Category = 'Digit' THEN ch END, '') AS Digits
FROM Classified;
...
--Task 15:
SELECT STRING_AGG(String, ' ') WITHIN GROUP (ORDER BY SequenceNumber) AS FullStatement
FROM DMLTable;
...
WITH ExperienceRank AS (
    SELECT 
        ...
        JobDescription,
        HireDate,
        RANK() OVER (PARTITION BY JobDescription ORDER BY HireDate) AS MostExperiencedRank,
        RANK() OVER (PARTITION BY JobDescription ORDER BY HireDate DESC) AS LeastExperiencedRank
    FROM Personal
)
SELECT 
    JobDescription,
...
SELECT 
    StudentID,
    FullName,
    Grade + ISNULL(LAG(Grade) OVER (ORDER BY ID), 0) AS AdjustedScore
FROM Students;

--TAsk 22:
WITH Hierarchy AS (
    -- Base case: the president (no manager)
    SELECT 
        EmployeeID,
        Name,
        ManagerID,
    ...
    -- Recursive case: each employee under a manager
    SELECT 
        e.EmployeeID,
        e.Name,
        e.ManagerID,
...
--Task 23:
-- Add TotalSum column if not already added
-- ALTER TABLE Equations ADD TotalSum INT;
...
BEGIN
    -- Build the dynamic SQL to evaluate the equation
    ...
    -- Update the result back into the table
    UPDATE Equations
    SET TotalSum = @Result
